...
## Important concept of OOP: Functions can respond in different ways depending on the object type.
...
```
### Summary called using different class of objects
...
### How does R distinguish types of var? what command can be used for this task?
...
```{r}
# usually the library calls have to be at the top. 
...
library(pryr)
is_s3_generic("print") # TRUE
is_s3_method("print") # FALSE
is_s3_method("print.Date") # TRUE
print
```
...
If no suitable methods can be found for a generic, then an error is thrown. For example, at the moment, get_n_elements() 
only has 2 methods available. If you pass a data.frame/matrix to get_n_elements() instead, you'll see an error. One could use generic.default to deal with all the missing class of objects.
...
extract_1_n_elements <- function(x, ...){
  useMethod("extract_n_1_elements")
}
```
